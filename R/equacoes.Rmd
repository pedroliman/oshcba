---
title: "Modelo Matemático - Calculadora SST/FPS"
...
### Fluxo de Caixa em Valor Presente (OK)
...
### Cálculo dos Benefícios da Iniciativa (OK)
...
#### Índices de Gravidade e Frequência Ampliados (OK)
...
### Ligação entre Eventos e Variáveis Previdenciárias (OK)
...
#### B91 - Auxílio Doença Acidentário (OK)
...
#### B92 - Aposentadoria por Invalidez Acidentária (OK)
...
#### B31 - Auxílio Doença Previdenciário (OK)
...
#### B32 - Aposentadoria Invalidez Previdenciário (OK)
...
#### Número de benefícios acumulados (OK)
...
Possibilidade 1: Todos os custos incorridos nesta rúbrica entram para o cálculo do FAP e não deveriam ser contados em duplicidade.
...
##### Reajustes do plano de Saúde (OK)
...
As despesas com interrupção operacional serão calculadas considerando o número de acidentes típicos, o tempo de interrução (o qual pode ser estimado pelo número total de dias de interrupção sobre o número total de acidentes) e o lucro cessante médio diário oriúndo de cada acidente.


Opção 1: Coletar os dias médios de interrupção geral.

Opção 2: Estimar os dias de interrupção de modo separado por tipo de acidentes (ex.: Óbito, Afastamento maior que 15, menor que 15, ou sem Afastamento.). Possívelmente, há como manter os dias de interrupção por óbito em separado.
...
Se 1= Sim e 2 = Não, então pode-se vincular os eventos de interdição com multa, incluindo uma probabilidade de multa gerar interdição.
Se 1 e 2 = Sim, então todos os eventos de multa gerarão interdição.
Se 1 e 2 = Não, o formato atual deve ser mantido.

$$D_{interdicao, fiscalizacao} = Evento_{interdicao} * (1+(f_{crise}*crise)) * dias_{interr,fiscalizacao} * lucrocessante $$

##### Reabilitação do Trabalhador (OK)
...
Opção 2: Criar 5 slots de números de multas a considerar. (Com este formato será possível segregar categorias de multa de acordo com a empresa, ou, no limite, usar apenas um dos slots).

Opção 3: Estimar custos médios de multa, e probabilidades de multa de modo individualizado (para cada NR).
...
4 alternativas pensadas para estimar a variável $D_{imagem, receita}$ são estas:

A) Ganho de Mercado Informado;

B) Ganho de Mercado Informado apenas quando se atende a uma taxa máxima de frequência e gravidade (considerando as taxas do INSS);

C) Regressão considerando os índices do INSS e PIB, explicando a variação das propostas ($Propostas * chance de conversão * Receita * margem média$);

D) Não fazer nada: Não inserir o ganho de receita via imagem no modelo.
...
##### Engajamento e Clima organizacional (OK)
...
##### Despesas com Turnover SST / FPS (OK)
...
Opção 2: Regressão a partir dos custos e índices de frequência e gravidade:
$$D_{qual,t} = var_{volumevenda,t} * margemmed_{unitario,t} * prod_{proj,t}$$
...
Opção 2: Regressão a partir dos custos e índices de frequência e gravidade:
O custo médio unitário será igual ao total produzido e os custo operacional.
