...
###### Percentis
Percentis são calculados de acordo com os índices nos dois anos anteriores.Os percentis dependem do posicionamento da empresa em relação às demais. Específicamente a função $Pos(I_{t})$ é calculada pela previdência de acordo com os índices de todas as empresas no mesmo subgrupo do CNAE da empresa em questão. **O calculo da Posição e Percentil é a área mais desafiadora para o calculo de uma projeção do FAP**. Será escolhida a alternativa "B". Curva aproximada a partir dos dados da empresa.
