...
resultados_simples = simular_cba("Dados.xlsx", output = "simples")
resultados_completo = simular_cba("Dados.xlsx", output = "completo")

linhas_esp = inputs$Configs$Replicacoes*(sum(inputs$Cenarios$Simular * 1)-1)

test_that("simular_cba( output = simples) retorna resultados como dataframe com numero de linhas correto", {
  expect_equal(nrow(resultados_simples), linhas_esp)
  expect_equal(class(resultados_simples)[4], "data.frame")
})

test_that("simular_cba(output = completo) retorna lista com numero de outputs correto", {
  expect_equal(length(resultados_completo), 7)
  expect_equal(class(resultados_completo), "list")
})

test_that("simular_cba( output = completo)$Resultados retorna resultados como dataframe com numero de linhas correto", {
